...
def get_dynamic_follow(v_ego, personality=log.LongitudinalPersonality.standard):
  ...
  if personality==log.LongitudinalPersonality.relaxed:
    x_vel =  [0.0,     8.3,     13.90,    20,      25]     #[0.0,  5.0,   13.90,  20,    25,    40]
    y_dist = [0.85,    0.85,    1.0,      1.15,    1.35]   #[1.2,  1.2,   1.5,   1.8,   2.2,  2.2]
  elif personality==log.LongitudinalPersonality.standard:
    x_vel =  [0.0,     8.3,     13.90,    20,      25]     #[0.0,  5.0,   13.90,  20,    25,    40]
    y_dist = [0.9,     0.9,     1.1,      1.25,    1.4]    #[1.1,  1.1,   1.3,    1.45,  1.6,  1.6]
  elif personality==log.LongitudinalPersonality.aggressive:
    x_vel =  [0.0,     8.3,     13.90,    20,      25]     #[0.0,  5.0,   12.00,  15,   20,    25,    40]
    y_dist = [0.95,    0.95,    1.2,      1.3,     1.5]    #[1.05, 1.10,  1.20,   1.20,  1.25,  1.25,   1.3]
  else:
...
class LongitudinalMpc:
  ...
  def update(self, radarstate, v_cruise, x, v, a, j, personality=log.LongitudinalPersonality.standard, dynamic_follow=False):
    ...
    self.smoother_braking = True if self.mode == 'acc' and np.any(v_ego < 20) and np.any(lead_xv_0[:,0] < 40) and not np.any(lead.dRel < (v_ego - 1) * t_follow) else False
