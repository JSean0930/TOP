...
def get_dynamic_follow(v_ego, personality=log.LongitudinalPersonality.standard):
  ...
  if personality==log.LongitudinalPersonality.relaxed:
    ...
    y_dist = [1.05,    1.1,      1.2,       1.35]   #[1.2,  1.2,   1.5,   1.8,   2.2,  2.2]
  elif personality==log.LongitudinalPersonality.standard:
    x_vel =  [0.0,     13.90,    20.84,      27.78]     #[0.0,  5.0,   13.90,  20,    25,    40]
...
class LongitudinalMpc:
  ...
  def update(self, radarstate, v_cruise, x, v, a, j, personality=log.LongitudinalPersonality.standard, dynamic_follow=False):
    ...
    self.smoother_braking = True if self.mode == 'acc' and np.any(v_ego < 15) and np.any(lead_xv_0[:,0] < 40) and not np.any(lead.dRel < (v_ego - 1) * t_follow) else False
